...
async def advertise():
    ...
    while True:
        ...
        if result:
            ...
            print(" --- Got door characteristic : ",
                  door_characteristic, "--- ")

            if door_characteristic is not None:
                print(" --- Reading to door characteristic with UUID:",
                      door_characteristic.uuid, "--- ")
                val = await door_characteristic.read()
                if len(val) == 1:
                    val += b'\x00'
                nounce_door = struct.unpack("<l", val)[0]
                print(" --- Get nouce from door characteristic : ",
                      nounce_door, "--- ")

                nounce_cle = random.randint(0, 100000000)
                print(" --- Writing to key characteristic hash with UUID:",
                      key_characteristic_hash.uuid, "--- ")
                string_to_hash = str(nounce_door) + password + str(nounce_cle)
                digest = hashlib.sha1(string_to_hash.encode()).digest()
                print(" --- Writing data:", digest, "--- ")
                key_characteristic_hash.write(digest)

                print(" --- Writing to key characteristic nounce with UUID:",
                      key_characteristic_nonce.uuid, "--- ")
